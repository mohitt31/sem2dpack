...
Instead, the true computational bottleneck is `mat_elast_f` (**35.45%** of runtime in this exact run; note this can vary by machine). Under the `OPT_NGLL=5` configuration, the code dispatches to `ELAST_KD2_PSV` which bypasses `mxmlib` entirely. It relies on `My_MATMUL`, a naive 5x5 triple loop with by-value array returns that creates significant memory overhead. The secondary bottleneck is the gather/scatter operations (`field_get_elem_sub` and `field_add_elem_2`) which consume an additional ~30% combined.

*(See the committed `EXAMPLES/2.5D_inplane/gprof_flat.txt` for the full verifiable output).*
